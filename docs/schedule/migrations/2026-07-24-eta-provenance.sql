...
SET @eta_ddl := IF(
    @eta_column_exists = 0,
    'ALTER TABLE schedule_push_job ADD COLUMN last_eta_source VARCHAR(30) NULL COMMENT ''LIVE_PROVIDER, SELECTED_ROUTE, or SAVED_FALLBACK'' AFTER last_live_fetched_at',
...
SET @eta_ddl := IF(
    @eta_column_exists = 0,
    'ALTER TABLE schedule_push_job ADD COLUMN last_traffic_change_minutes INT NULL COMMENT ''Last comparable live-to-live ETA delta in minutes'' AFTER last_eta_failure_reason',
...
-- Upgrade verification: expected_count must be 6 and every row must have the expected type.
SELECT
    column_name,
    column_type,
    is_nullable,
