...
- 알림 클릭 후 일정 상세 이동을 실제 router에 연결
- push payload type별 UX
...
flowchart TD
  ...
  E --> E1["완료: 토큰 등록 / foreground 표시 / scheduleId 추출"]
  E --> E2["다음: payload별 라우팅 / 알림 액션"]
...
1. 알림 클릭 callback에서 expo-router 일정 상세 이동 연결
2. `SCHEDULE_TRAFFIC` payload 수신 시 상세 이동 테스트
