...
- `departNow=true` 일정 알림에 출발 완료 액션 연결
- 출발 완료 액션에서 `POST /api/schedules/{scheduleId}/depart-now` 호출
- iOS TestFlight 업로드 경로와 production APS entitlement 확인
...
- 출발 완료 액션 후 BE PushJob 취소 실기기 검증
...
1. TestFlight 최신 빌드에서 `SCHEDULE_TRAFFIC` / `SCHEDULE_DEPARTURE_REMINDER` 클릭 상세 이동 확인
2. `departNow=true` 알림의 출발 완료 액션과 BE PushJob 취소 확인
