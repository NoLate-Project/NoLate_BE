...
class TmapTrafficClientTest {
    ...
    @Test
    fun `BIKE와 ETC는 자동차 API를 호출하지 않고 명시적 fallback을 반환한다`() {
        ...
        val etc = client.getTravelMinutes(request(mode = ScheduleTravelMode.ETC))
        ...
        assertEquals(TrafficSource.SAVED_FALLBACK, etc.source)
        assertTrue(etc.failureReason.orEmpty().contains("ETC"))
    }
    ...
    fun `선택 대중교통 여정은 다른 추천 여정을 LIVE로 위장하지 않고 snapshot을 사용한다`() {
        ...
        assertEquals(TrafficFailureReasons.SELECTED_TRANSIT_ROUTE_NOT_REFRESHABLE, result.failureReason)
    }
