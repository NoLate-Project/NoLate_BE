...
class ScheduleNotificationActionIdempotencyIntegrationTest @Autowired constructor(
    ...
    fun setUp() {
        ...
        whenever(departureStatusService.markDeparted(any(), any())).thenReturn(
    ...
    fun `concurrent same snooze key mutates exactly once and returns persisted result`() {
        ...
        repeat(2) {
            executor.submit {
                ...
                start.await()
                runCatching {
                    service.snooze(memberId, scheduleId, snoozeKey)
    ...
    fun `response loss retry and persistence context restart do not move snooze again`() {
        val first = service.snooze(memberId, scheduleId, snoozeKey)
        entityManager.clear()

        // 첫 응답을 FE가 받지 못하고 프로세스가 다시 올라온 뒤 같은 key를 보낸 상황과 같다.
        val retried = service.snooze(memberId, scheduleId, snoozeKey)
    ...
    fun `depart now retry returns authoritative current result without reapplying mutation`() {
        assertEquals(scheduleDto, service.departNow(memberId, scheduleId, departKey))
        assertEquals(scheduleDto, service.departNow(memberId, scheduleId, departKey))

        verify(departureStatusService, times(1)).markDeparted(memberId, scheduleId)
    ...
    fun `same key cannot cross member schedule or action scope`() {
        ...
        listOf(
            { service.snooze(memberId + 1, scheduleId, snoozeKey) },
            { service.snooze(memberId, scheduleId + 1, snoozeKey) },
            // 동일 raw key를 다른 action endpoint에 재사용하면 기존 receipt scope가 우선해 409다.
            { service.departNow(memberId, scheduleId, snoozeKey) },
        ).forEach { request ->
    ...
    fun `invalid or PII-shaped key is rejected before persistence`() {
        listOf(
            ...
            val failure = assertThrows<BusinessException> {
                service.snooze(memberId, scheduleId, invalidKey)
            }
