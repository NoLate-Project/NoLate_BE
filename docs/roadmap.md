...
11. 후속 외부 연동: 서울 지하철 실시간 도착 인증키는 2026-08-04 로컬 비밀 설정에 연결했고,
    강남역 실서버 조회에서 `INFO-000`과 도착정보 5건을 확인했다. 별도의
    `서울특별시_정류소정보조회 서비스` 개발키도 로컬 비밀 설정에 연결했으며 Encoding/Decoding
    키를 모두 정확히 한 번 인코딩하도록 버스 클라이언트를 보강했다.
    다만 실서버는 현재 `SERVICE KEY IS NOT REGISTERED`(headerCd 7)를 반환하므로 공공데이터포털
    활용 승인 전파를 확인한 뒤 서울 버스 실시간 도착정보를 활성화한다. 운영 완료 조건은
    비밀 저장소를 통한 `SEOUL_BUS_API_KEY` 주입, 신뢰된 HTTPS egress 경계,
