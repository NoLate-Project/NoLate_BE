...
- 일정 조회는 FK 없는 DB durable revision과 `owned/shared` 범위별·월 단위 Redis payload 캐시를 사용하고 `GET /api/schedules/calendar-cache/revision`으로 클라이언트 동기화 지원
- 일정 생성·수정·삭제, 직접 일정/카테고리 공유, 캘린더 멤버·역할·설정·소유권·보관 변경은 transaction-local audience를 합쳐 terminal `BEFORE_COMMIT`에 revision row를 ID 오름차순 한 번만 잠가 원자 무효화
...
2026-07-24 최초 캐시 및 2026-07-27 durable revision 전환 기록:

- Redis는 범위별 월 payload와 15분 data TTL만 담당하고, FK 없는 `schedule_calendar_cache_revisions`가 재시작·eviction에도 유지되는 authority를 담당
- 일정/카테고리/캘린더 공유 및 초대 수락, 역할·설정·소유권·보관 변경의 영향 회원을 transaction 단위로 합쳐 동일 commit에서 durable revision 증가
- 운영은 old writer 전부 중단 후 migration/backfill/marker를 적용하는 atomic cutover이며 mixed-version rolling과 legacy v1 cache-on binary rollback 금지
