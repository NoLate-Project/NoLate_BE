...
class ScheduleNotificationActionRetryUnitTest {
    ...
    private val sessionGeneration = 3L
    ...
    fun `transient lock failure retries in fresh writer transactions and converges`() {
        ...
        whenever(writer.snooze(eq(7L), eq(9L), any(), eq(sessionGeneration)))
        ...
        val result = ScheduleNotificationActionIdempotencyService(writer)
            .snooze(7L, 9L, rawKey, sessionGeneration)

        assertEquals(expected, result)
        verify(writer, times(3)).snooze(eq(7L), eq(9L), any(), eq(sessionGeneration))
    }
    ...
    fun `expected receipt duplicate retries in a fresh transaction and converges`() {
        ...
        assertEquals(expected, result)
        verify(writer, times(2)).snooze(eq(7L), eq(9L), any(), eq(sessionGeneration))
    }
    ...
    fun `generic data integrity violation is not retried or hidden`() {
        ...
        whenever(writer.snooze(eq(7L), eq(9L), any(), eq(sessionGeneration))).thenThrow(expected)
        ...
        verify(writer, times(1)).snooze(eq(7L), eq(9L), any(), eq(sessionGeneration))
    }
